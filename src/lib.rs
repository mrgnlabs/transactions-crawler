...
impl CrawlerConfig {
    pub fn new(targets: Vec<Target>) -> Self {
        Self {
            ...
            signature_fetch_limit: 1_000,
            max_concurrent_requests: 10,
            max_pending_signatures: 10_000,
            monitor_interval: 5,
            targets,
        }
    }
...
pub struct TransactionCrawler {
    context: Context,
}

impl TransactionCrawler {
    pub fn new(targets: Vec<Target>) -> Self {
        let config = CrawlerConfig::new(targets);
        let context = Context::new(&config);
        Self { context }
    }

    pub fn new_with_config(config: CrawlerConfig) -> Self {
        let context = Context::new(&config);
        Self { context }
    }
    ...
    where
        F: (FnOnce(Arc<Context>) -> Fut) + Clone + Send + Sync + 'static,
    ...
    async fn crawl_signatures_for_address(ctx: Arc<Context>) {
        ...
        loop {
            ...
            let minimim_available_slot_future = ctx.rpc_client.minimum_ledger_slot();

            let (signatures_result, minimim_available_slot_result) =
                join!(signatures_futures, minimim_available_slot_future);
            ...
            *ctx.minimum_slot_available.lock().unwrap() = minimim_available_slot_result.unwrap();
    ...
    async fn fetch_transactions(ctx: Arc<Context>) {
        loop {
            let mut signatures = vec![];
            {
                let mut timing = Measure::start("Q_pop_lock_wait");
            ...
            if !signatures_to_retry.is_empty() {
                // This can go over the soft limit on the sig queue. NMot an issue unless most fetch's fail which would point to bigger issues
                warn!(
                ...
                let mut timing = Measure::start("Q_push_retries_lock_wait");
                let signature_queue = ctx.signature_queue.lock().unwrap();
                timing.stop();
                if timing.as_ms() > 0 {
                    warn!("{}", timing);
